...
def extract_gait_features_io(path_preprocessed_input: Union[str, Path], path_output: Union[str, Path], config: GaitFeatureExtractionConfig) -> None:
    # Load data
    metadata_time, metadata_values = read_metadata(path_preprocessed_input, config.meta_filename, config.time_filename, config.values_filename)
    ...
    write_df_data(metadata_time, metadata_values, path_output, 'gait_meta.json', df_features)


def detect_gait(df: pd.DataFrame, config: GaitDetectionConfig, path_classifier_input: Union[str, Path]) -> pd.DataFrame:
    """
    ...
    path_classifier_input : Union[str, Path]
    ...
    clf = pd.read_pickle(os.path.join(path_classifier_input, 'classifiers', config.classifier_file_name))

    # Load and apply scaling parameters
    with open(os.path.join(path_classifier_input, 'scalers', 'gait_detection_scaler_params.json'), 'r') as f:
...
def extract_arm_activity_features(
    ...
    with open(os.path.join(path_classifier_input, 'thresholds', gait_detection_config.thresholds_file_name), 'r') as f:
    ...
    df[config.angle_colname], df[DataColumns.VELOCITY] = compute_angle_and_velocity_from_gyro(config, df)
    ...
    windowed_cols = (
        ...
        [config.angle_colname, DataColumns.VELOCITY]
    )
...
def extract_arm_activity_features_io(path_timestamp_input: Union[str, Path], path_prediction_input: Union[str, Path], path_classifier_input: Union[str, Path], path_output: Union[str, Path], config: ArmActivityFeatureExtractionConfig) -> None:
    ...
    for sensor in ['accelerometer', 'gyroscope']:
        ...
        metadata_ts_dict = tsdf.load_metadata_from_path(os.path.join(path_timestamp_input, meta_ts_filename))
    ...
    metadata_pred_dict = tsdf.load_metadata_from_path(os.path.join(path_prediction_input, meta_pred_filename))
    ...
    df_features = extract_arm_activity_features(df_ts, df_pred_gait, config, path_classifier_input)
    ...
    write_df_data(metadata_ts_time, metadata_ts_values, path_output, 'arm_activity_meta.json', df_features)


def filter_gait(df: pd.DataFrame, config: FilteringGaitConfig, path_classifier_input: Union[str, Path]) -> pd.DataFrame:
    """
    ...
    # Load and apply scaling parameters
    with open(os.path.join(path_classifier_input, 'scalers', 'gait_filtering_scaler_params.json'), 'r') as f:
        scaler_params = json.load(f)
...
def filter_gait_io(path_feature_input: Union[str, Path], path_classifier_input: Union[str, Path], path_output: Union[str, Path], config: FilteringGaitConfig) -> None:
    # Load the data
    metadata_time, metadata_values = read_metadata(path_feature_input, config.meta_filename, config.time_filename, config.values_filename)
    df = tsdf.load_dataframe_from_binaries([metadata_time, metadata_values], tsdf.constants.ConcatenationType.columns)

    df = filter_gait(df, config, path_classifier_input)
    ...
    write_df_data(metadata_time, metadata_values, path_output, 'arm_activity_meta.json', df)


def quantify_arm_swing(df_features: pd.DataFrame, df_predictions: pd.DataFrame, config: ArmSwingQuantificationConfig, path_classifier_input: Union[str, Path]) -> pd.DataFrame:
    ...
    with open(os.path.join(path_classifier_input, 'thresholds', filtering_gait_config.thresholds_file_name), 'r') as f:
        classification_threshold = float(f.read())
...
def quantify_arm_swing_io(path_feature_input: Union[str, Path], path_prediction_input: Union[str, Path], path_classifier_input: Union[str, Path], path_output: Union[str, Path], config: ArmSwingQuantificationConfig) -> None:
    ...
    metadata_dict = tsdf.load_metadata_from_path(os.path.join(path_prediction_input, config.meta_filename))
    ...
    df_aggregates = quantify_arm_swing(df_features, df_predictions, config, path_classifier_input)

    # Store data as json
    with open(os.path.join(path_output, 'aggregates.json'), 'w') as f:
