...
The final dataframe should have the correct units for the sensor columns and the correct format for the time column. Note that _ParaDigMa automatically handles resampling_ - data does not need to be pre-resampled. Frequencies at 50 Hz or higher are typically used directly without upsampling to avoid interpolation artifacts. If your data is at a lower frequency (e.g., 30 Hz), ParaDigMa will automatically upsample to 100 Hz. Also note that the _gait_ pipeline expects a specific orientation of sensor axes, as explained in [Coordinate system](../guides/coordinate_system).
